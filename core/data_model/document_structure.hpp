...
{
    success = 0u, invalid_input_view, syntax_error, allocation_failed, scratch_size_limit, internal_error
};
...
{
    EDocumentStructureStatus status{ EDocumentStructureStatus::invalid_input_view };
    document_text::ESyntaxError syntax_error{ document_text::ESyntaxError::none };
    //  Byte offset in the linter's UTF-8 output, not the original input. EOF
    //  errors use the logical byte size. Zero on success/invalid view.
...
//  its physical terminal zero and preserve quoted content during linting.
